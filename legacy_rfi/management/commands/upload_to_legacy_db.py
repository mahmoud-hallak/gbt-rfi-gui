...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		self.directory = "/home/www.gb.nrao.edu/content/IPG/rfiarchive_files/GBTDataImages/"
	...
	def parse_header(self):
		header_rows = pd.read_table(self.directory+self.session, delimiter=":",
			names = ["names", "data"], skiprows= 1, nrows=25)
		...
		data_string = "################   Data  ################"
		header_rows = header_rows[:header_rows[header_rows['names'] == data_string].index[0]]

		for i in header_rows["names"]:
			header_rows["names"] = header_rows["names"].replace(i, i.split(" ")[1].strip())

		change_values = {'utc':'utc_hrs', 'scan_numbers':'scan_number',
			'number_IF_Windows':'number_if_windows', 'frequency_resolution':'resolution_mhz',
			'azimuth':'azimuth_deg', 'elevation':'elevation_deg'}
	...
	def parse_data(self):
		...
		data_rows = pd.read_fwf(self.directory+self.session, skiprows=22,
			names=["window", "channel", "frequency_mhz", "intensity_jy"], index_col=False,
			colspecs=col_specs)
	...
	def upload_sessions(self, data):
		...
		for i in range(0, len(data.index), chunk_size):
			...
			for index, s in data.iloc[i : i + chunk_size].iterrows():
				sessions_data.append(
				    MasterRfiCatalog(
				    	window = s.window,
				    	channel = s.channel,
				    	frequency_mhz = s.frequency_mhz*1000,
			...
			uploaded_sessions = MasterRfiCatalog.objects.bulk_create(sessions_data, batch_size=chunk_size)
