...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def do_plot(
        ...
        data = pd.DataFrame(
            qs.values("frequency", "intensity", "scan__datetime", "scan__session__name","is_peak")
        )
    ...
    def dynamic_query(self, freq_min, freq_max, sessions,zoom):
        ...
        print(zoom)
        if(zoom == 0):
            qs.filter(view_level_0=True)
        elif (zoom == 1):
            qs.filter(view_level_1=True)
        elif(zoom == 2):
            qs.filter(view_level_2=True)
    ...
    def make_plot(
        ...
        def on_lims_change(event_ax):
            ...
            df_freq_max = self.interval_data["frequency"].max()

            df_freq_min = self.interval_data["frequency"].min()

            pts_per_freq = len(self.interval_data["frequency"]) / (
                df_freq_max - df_freq_min
            )

            displayed_pts = pts_per_freq * (freq_max - freq_min)

            #print("max: " + str(df_freq_max) + "  min: " + str(df_freq_min) )

            print("Array length bef: " + str(len(self.interval_data["frequency"])))

            #problem in it refreshing is probably here the pts_per_freq should be changed
            ...
            if 1.2 >= (displayed_pts / windowsize):
                ...
                self.interval_data = self.dynamic_query(freq_min, freq_max, sessions, 1)

                pts_per_pixel = len(self.interval_data["frequency"]) / windowsize


                if 1.2 >= (pts_per_pixel / windowsize):
                    
                    self.interval_data = self.dynamic_query(freq_min, freq_max, sessions, 2)

                    pts_per_pixel = len(self.interval_data["frequency"]) / windowsize


                    if 1.2 >= (displayed_pts / windowsize):
                        self.interval_data = self.dynamic_query(freq_min, freq_max, sessions, 3)
                

                # plt.fill_between(second_filtered_data["frequency"], second_filtered_data["intensity_mean"], color="black")

                pts_per_pixel = len(self.interval_data["frequency"]) / windowsize
            elif (freq_max - freq_min) == first_filtered_data["frequency"].max() - first_filtered_data["frequency"].min():
