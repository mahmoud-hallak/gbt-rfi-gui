...
class QueryForm(forms.Form):
    ...
    freq_high = forms.FloatField(
        label="High Frequency",
        required=False,
        error_messages={"freq_low": "Freq_high is lower than freq_low"},
        widget=forms.TextInput(),
    )
    freq_low = forms.FloatField(
        label="<hr> Low Frequency",
        required=False,
        error_messages={"too_low": "Your high is too low"},
        widget=forms.TextInput(),
    )
    date = forms.DateField(
        label="<hr> <center> Specify either Project Date of interest <b>OR</b> Date Range below. </center> Project Date",
        required=False,
    ...
    def clean(self, num_of_pts=0):
        ...
        if low and high:
            if not float(low):
                self.add_error(
                    "freq_low", forms.ValidationError("Frequencies must be numerical")
                )
            if not float(high):
                self.add_error(
                    "freq_high", forms.ValidationError("Frequencies must be numerical")
                )

            if low == high:
                self.add_error(
                    "freq_low", forms.ValidationError("Frequencies cannot be the same")
                )
                self.add_error(
                    "freq_high", forms.ValidationError("Frequencies cannot be the same")
                )
        ...
        if end and start:
            if end < start:
                self.add_error(
                    "start",
                    forms.ValidationError("End date must be later than start date"),
                )
                self.add_error(
                    "end",
                    forms.ValidationError("End date must be later than start date"),
            ...
            if (end - start) > MAX_TIME_SPAN:
                self.add_error(
                    "start", forms.ValidationError("Max time span is 1 year.")
                )
                self.add_error("end", forms.ValidationError("Max time span is 1 year."))

        if (date and start) or (date and end):
            self.add_error(
                "start", forms.ValidationError("Specify a project date OR date range")
            )
            self.add_error(
                "end", forms.ValidationError("Specify a project date OR date range")
            )
            self.add_error(
                "date", forms.ValidationError("Specify a project date OR date range")
            )
